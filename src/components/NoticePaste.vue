...
function analyze() {
  ...
  parsed.value = parseNotice(source.value, props.courses)
  matches.value = findNoticeChanges(parsed.value, props.tasks)
  if (matches.value.length) {
    selectedId.value = matches.value[0].task.id
    mode.value = 'update'
  } else {
    selectedId.value = ''
    mode.value = 'create'
  ...
  <Modal :open="open" title="📋 粘贴通知" wide @close="emit('close')">
    <div class="notice-layout">
      <section class="source-panel">
        <label>老师或班群通知</label>
        <textarea v-model="source" rows="8" placeholder="例如：下周三晚上八点前提交实验报告，文件名为学号姓名。"></textarea>
      ...
      <section class="preview-panel">
        ...
        <template v-else>
          <div class="preview-head"><div><span>PARSED RESULT</span><h3>确认解析结果</h3></div><span v-if="!parsed.dueDate" class="warn">未识别日期</span></div>
          <div class="preview-form">
            <label>待办标题<input v-model="parsed.title" /></label>
            <div class="row"><label>截止日期<input v-model="parsed.dueDate" type="date" /></label><label>截止时间<input v-model="parsed.dueTime" type="time" /></label></div>
