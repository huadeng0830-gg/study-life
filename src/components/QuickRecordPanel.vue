...
function chooseTime(draft, value) {
  draft.time = value
  draft.questions = draft.questions.filter((item) => item.field !== 'time')
}
...
<template>
  <Modal :open="open" title="⚡ 快速记录" medium @close="emit('close')">
    <section class="quick-record">
      ...
      <p class="example">午饭18元 / 周五交高数作业 / 明天下午三点组会</p>
      ...
      <section v-if="hasDrafts" class="results" aria-live="polite">
        ...
        <article v-for="draft in drafts" :key="draft.id" class="record-card">
          ...
          <input v-model="draft.title" class="title-edit" aria-label="记录标题" />
          <div class="chips">
          ...
          <div v-for="question in draft.questions" :key="question.field" class="question"><span>⚠ {{ question.label }}</span><button v-for="choice in question.choices" :key="choice" type="button" @click="chooseTime(draft, choice)">{{ choice }}</button></div>
